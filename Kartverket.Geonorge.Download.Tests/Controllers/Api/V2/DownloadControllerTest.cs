...
namespace Kartverket.Geonorge.Download.Tests.Controllers.Api.V2
{
    public class DownloadControllerTest
    {
        ...
        private void SetupRequestingUser(string orderUsername)
        {
            var claims = new List<Claim> {new Claim("username", orderUsername)};
            Thread.CurrentPrincipal = new ClaimsPrincipal(new ClaimsIdentity(claims, "testauth"));
        }


        private static DownloadController CreateDownloadController(IOrderService orderServiceMock, IDownloadService downloadServiceMock)
        {
            var downloadController = new DownloadController(orderServiceMock, downloadServiceMock);
        ...
        {
            var downloadController = CreateDownloadController(CreateOrderServiceMock(AccessConstraint.NorgeDigitalRestricted).Object, null);
            var result = downloadController.GetFile(OrderUuid, FileId) as RedirectResult;
            result.Should().NotBeNull();
            result?.Location.ToString().Should().Contain("SignIn"); // kentor authservices uses SignIn in url for login page
        }
        ...
        {
            SetupRequestingUser("anotherUsername");
            var downloadController = CreateDownloadController(CreateOrderServiceMock(AccessConstraint.NorgeDigitalRestricted).Object, null);
        ...
        {
            SetupRequestingUser(OrderUsername);
            var downloadController = CreateDownloadController(CreateOrderServiceMock().Object, null);
        ...
        {
            var downloadController = new DownloadController(new Mock<IOrderService>().Object, null);
        ...
        public void ShouldReturnNotFoundWhenOrderItemIsNotReadyToBeDownloaded()
        {
            ...
            var downloadController = new DownloadController(orderService, null);
