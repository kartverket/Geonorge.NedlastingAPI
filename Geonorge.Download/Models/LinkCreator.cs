...
namespace Geonorge.Download.Models
{
    public class LinkCreator
    {
        ...
        public List<LinkType> CreateCapabilityLinks(IConfiguration config, HttpRequest request, string metadataUuid)
        {
            var links = CreateLinks(config, request, metadataUuid);
        ...
        public List<LinkType> CreateLinks(IConfiguration config, HttpRequest request)
        {
            return CreateLinks(config, request, null);
        }

        private List<LinkType> CreateLinks(IConfiguration config, HttpRequest request, string metadataUuid)
        {
            var apiBaseUrl = GetApiBaseUrl(config, request);
            ...
            var links = new List<LinkType>
            {
                ...
                CreateCapabilitiesLink(request, metadataUuid, apiBaseUrl),
                CreateCanDownloadLink(apiBaseUrl)
            };
            return links;
        }

        public LinkType[] CreateOrderReceiptLinks(IConfiguration config, HttpRequest request, Guid orderUuid)
        {
            var apiBaseUrl = GetApiBaseUrl(config, request);
        ...
        private static LinkType CreateCapabilitiesLink(HttpRequest request, string metadataUuid, string apiBaseUrl)
        {
            return new LinkType
            {
                rel = RelCapabilities,
                href = GetCapabilitiesUrl(request, metadataUuid, apiBaseUrl)
            };
        }
        ...
        private static string GetCapabilitiesUrl(HttpRequest request, string metadataUuid, string apiBaseUrl)
        {
        ...
        private static string GetApiBaseUrl(IConfiguration config, HttpRequest request)
        {
            return config["DownloadUrl"] + "api/" + GetVersion(request);
        }
